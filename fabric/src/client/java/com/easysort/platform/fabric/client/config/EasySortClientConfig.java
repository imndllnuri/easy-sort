package com.easysort.platform.fabric.client.config;
...
public final class EasySortClientConfig {
	...
	private static final Path FILE = FabricLoader.getInstance().getConfigDir().resolve("easy-sort.json");
	...
	public static void load() {
		...
		try (Reader reader = Files.newBufferedReader(FILE)) {
			...
			EasySort.LOGGER.warn("Failed to load Easy Sort config, using defaults", e);
		}
	}
	...
	private static void save() {
		try {
			...
			EasySort.LOGGER.warn("Failed to save Easy Sort config", e);
		}
	}
